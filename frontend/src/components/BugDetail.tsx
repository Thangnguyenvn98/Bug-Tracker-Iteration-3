...
const BugDetail = () => {
  ...
  const onSubmit: SubmitHandler<BugFormDetail> = async (data) => {
    ...
    try {
        ...
        const errorMessage = e instanceof AxiosError ? e.response?.data.message : 'Edit bug failed';
  ...
  return (
    <div className="p-4">
            ...
            <h1 className="text-2xl font-bold text-center mb-5 mt-6">Edit Bug Report</h1>
                            ...
                            {isCloseBugChecked && (
                                    <>
                                        <div className="flex gap-x-[1.5rem]">
                                            ...
                                            <textarea {...register('reasonForClosing', { required: true })} rows={2} cols={30} className="border-2 border-slate-400 text-black" ></textarea>
                                {errors.reasonForClosing && <span className="text-red-500">Closing Reason is Required</span>}
                                        </div>
                                        <div className="flex gap-x-4">
                                 ...
                                 {isBugFixedChecked && isCloseBugChecked && (
                                        ...
                                        <div className="flex gap-x-4">
                                            ...
                                            <textarea {...register('bugFixDetails', { required: true })} rows={8} cols={30} className="border-2 border-slate-400 text-black" ></textarea>
                                {errors.bugFixDetails && <span className="text-red-500">How bug is fixed Required</span>}
                                        </div>
                                        
                                    
                                )}
                            <button type="submit" className="bg-white text-blue-500 font-bold py-2 px-4 rounded">Save and Exit</button>
                        </div>
                    </form>
                </div>
