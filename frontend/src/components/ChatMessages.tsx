...
import { Bot, Loader2, ServerCrash, ShieldCheck, UserRound } from 'lucide-react';
...
const ChatMessages:React.FC<ChatMessagesProps> = ({roomId,username,currentRoom}) => {
    const { data, fetchNextPage, hasNextPage, isFetchingNextPage, status } = useChatQuery(roomId);
      ...
      <div ref={chatRef}  className="flex-1 py-4 overflow-y-auto flex flex-col">
        ...
        <div className="text-white flex flex-col-reverse mt-auto flex-1 overflow-y-auto">
          {data?.pages.map((group, index) => (
            <Fragment key={index}>
              {Array.isArray(group.messages) && group.messages.map((message: Message) => (
                <div
                  key={message._id}
                  className={cn(
                    'rounded-lg p-2 my-4 inline-block break-words w-full max-w-[300px]',
                    message.username === username ? 'ml-auto mr-2 bg-blue-500 text-right' : 'mr-auto ml-2 bg-chat text-left'
                  )}
                >
                  <span className={message.username === username ? 'mr-2' : 'ml-2'}>{message.message || ''}</span>
                  {message.imageUrl && (
           
                <Button className="p-1 flex bg-transparent hover:bg-transparent justify-center items-center overflow-hidden max-h-[400px] h-auto" 
              ...
              className="max-h-full max-w-full object-contain cursor-pointer"
                />
                  ...
                  <div className={cn("flex text-sm items-center", message.username === username ? 'justify-end pr-2' : 'justify-start pl-2')}>
