...
const RequestReset = () => {
  ...
  const onSubmit: SubmitHandler<ResetRequestProps> = async (data) => {
    try {
      ...
      const errorMessage =
        e instanceof AxiosError ? e.response?.data.message : "Fail to request";
      ...
      <div className="p-10 flex justify-center">
        <div className="p-4 bg-white w-[400px] rounded-lg border-2 border-black">
          ...
          <p className="text-center text-md mt-4">
            Please enter the email you use to sign in to BRS
          </p>
          <form onSubmit={handleSubmit(onSubmit)} className="p-4">
            <div className="flex flex-col gap-y-6">
              ...
              <Link to="/sign-in" className="text-blue-500 text-center">
                Back to Sign in
              </Link>
            </div>
          </form>
