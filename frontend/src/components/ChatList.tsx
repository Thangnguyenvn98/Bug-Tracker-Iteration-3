...
const ChatList:React.FC<ChatListProps> = ({roomId,userId,username,rooms,socket}) => {
  ...
  return (
        ...
        <Input className="bg-slate-200 pl-10"   value={searchTerm}
                        onChange={(e) => setSearchTerm(e.target.value)} type="text" placeholder='Search Community Chat' />
        </div>
        ...
        {filteredRooms.map((room, index) => (
                    <div
                        ...
                        className={cn('flex flex-col p-2 rounded-md cursor-pointer hover:bg-slate-100', room._id === roomId ? 'bg-chat-color text-white hover:bg-chat-color' : '')}
                    >
