...
const BugLibrary = () => {
  ...
  const handleReportNumberSubmit = async (
    event: React.FormEvent<HTMLFormElement>
  ) => {
    event.preventDefault();
    const reportExists = reports.find(
      (report) => report.number.toString() === reportNumber
    );
    if (reportExists) {
  ...
  return (
    <div className="p-4">
      ...
      <form
        onSubmit={handleReportNumberSubmit}
        className="mb-4 flex justify-center gap-x-2"
      >
        <input
          type="text"
          value={reportNumber}
